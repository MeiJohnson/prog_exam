...
def find_popular_name(data):
    ...
    for i in range(humanValue):
        if (sex_lst[i] == 'male'):
            temp_name = names_lst[i].split('. ')[1] # Выбираем часть с именем
...
def find_popular_adult_names(data):
    ...
    ages_lst = data['Age'].tolist()
    names_lst = data['Name'].tolist()
    sex_lst = data['Sex'].tolist()


    for i in range(humanValue):
        if (sex_lst[i] == 'male' and ages_lst[i] > 15):
            male_temp_name = names_lst[i].split('. ')[1] # Выбираем часть с именем
            male_temp_names = male_temp_name.split(' ') # создаем список составных частей имени
            k = 0
            while (k < len(male_temp_names)):
                if (not(male_names_dict.get(male_temp_names[k]))):
                    male_names_dict.update({male_temp_names[k]:0}) # добавляем каждое имя в словарь с нулевым значением
                    k+=1  
        elif (sex_lst[i] == 'female' and ages_lst[i] > 15):
            female_temp_name = names_lst[i].split('. ')[1] # Выбираем часть с именем
            female_temp_names = female_temp_name.split(' ') # создаем список составных частей имени
            while (k < len(female_temp_names)):
                if (female_temp_names[k].find('(')):
                    temp_str = female_temp_names[k].split('(')
                    female_temp_names.remove(female_temp_names[k])
                    female_temp_names.extend(temp_str)
                    if (not(female_names_dict.get(temp_str[0]))):
                        female_names_dict.update({temp_str[0]:0})
                else if (female_temp_names[k].find(')')): #??????????
                    temp_str = female_temp_names[k].split(')')
                    print(temp_str)
                    temp_str.remove('')
                    female_temp_names.remove(female_temp_names[k])
                    ...
                    if (not(female_names_dict.get(female_temp_names[k]))):
                        female_names_dict.update({female_temp_names[k]:0})
                k+=1 
    ...
    for i in range(humanValue): # считаем количество повторений имен
        if (sex_lst[i] == 'male'):
                if (len(temp_name.split(' ')) > 1):
                    male_temp_names = male_temp_name.split(' ')
                    male_names_dict[male_temp_names[0]] +=1
                    male_names_dict[male_temp_names[1]] +=1
                else:
                    male_names_dict[temp_name] +=1
        if (sex_lst[i] == 'female'):
                if (len(female_temp_name.split(' ')) > 1):
                    female_temp_names = female_temp_name.split(' ')
                    female_names_dict[female_temp_names[0]] +=1
                    female_names_dict[female_temp_names[1]] +=1
                else:
                    female_names_dict[temp_name] +=1
     

    male_names_sorted = sort_dict(male_names_dict)
    popular_male_name = male_names_sorted.popitem()[0]
    female_names_sorted = sort_dict(female_names_dict)
    popular_female_name = female_names_sorted.popitem()[0]
